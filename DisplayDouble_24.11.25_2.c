...
int main(void) {
    ...
    int offsetdpp = 2; //Смещения для буфера
    int offset1 = 0; //Смещения для буфера
    int offset2 = 0;
    ...
    if ((sourcenum > -1.000) && (sourcenum <= -0.100)) {
        offset1 = 3;
        offset2 = 2;
    }
    if ((sourcenum > -0.100) && (sourcenum <= -0.01)) {
        offset1 = 4;
        offset2 = 3;
    }
    if ((sourcenum > -0.010) && (sourcenum <= -0.001)) {
        offset1 = 5;
        offset2 = 4;
    }
    ...
    while (1) {
        ...
        char buffer[8] = {};
        int dppnew = dpp + offsetdpp; //Для вычисления правильного положения точки (сдвигается вправо)
        for (int k = 0; k < sizeof(buffer); k++) {
            ...
            if (k > dppnew) {
                buffer[k] = myarray[k - offset1] + '0';
            }
            if ((k < dppnew) && (k > 0)) {
                buffer[k] = myarray[k - offset2] + '0';
            }
        }
